...
# Lista completa de padroes padrao
    ...
    while True:
        ...
        if mode == "custom":
            ...
            if patterns:
                ...
                return "default", DEFAULT_IGNORE_PATTERNS
...
def push_to_existing_repo(folder, repo_info, username, include_env, gitignore_patterns=None):
    ...
    try:
        ...
        with Progress(
            ...
            progress.update(t, description="[cyan]Criando commit...")
            run_git(["commit", "-m", "Update via GitAutomatico"], clone_dir, check=True)

            progress.update(t, description=f"[cyan]Enviando para origin/{default_branch}...")
            push = run_git(["push", "origin", default_branch], clone_dir)
            if push.returncode != 0:
...
def flow_existing(username):
    ...
    for i, r in enumerate(repos[:15], 1):
        ...
        table.add_row(str(i), r["name"], vis, upd)
    console.print(table)
    if len(repos) > 15:
    ...
    console.print(Panel(
        ...
        f"[bold]Branch:      [/bold]  {selected.get('default_branch') or 'main'}\n"
        f"[bold]Visib.:     [/bold]  {'privado' if is_private else 'publico'}\n"
        f"[bold].env:        [/bold]  {env_status}\n"
        f"[bold]Gitignore:  [/bold]  {'padrao' if _ig_mode == 'default' else f'personalizado ({len(gitignore_patterns)} padroes)'}\n"
        f"[bold]Extras:     [/bold]  README gerado se nao existir",
    ...
    ok, msg = push_to_existing_repo(folder, selected, username, include_env, gitignore_patterns)
    console.print()
    if ok:
        if msg == "no_changes":
    ...
    while True:
        action = questionary.select(
            ...
            choices=[
                ...
                questionary.Choice("Gerenciar lista gitignore personalizada", value="gitignore"),
...
def _manage_gitignore():
    """Menu para gerenciar a lista personalizada de gitignore."""
    op = questionary.select(
        "Gerenciar gitignore:",
        choices=[
            questionary.Choice("Visualizar lista padrao",         value="view_default"),
            questionary.Choice("Editar lista personalizada salva", value="edit"),
            questionary.Choice("Apagar lista personalizada salva", value="delete"),
